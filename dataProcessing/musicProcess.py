...
class MusicModule:
    ...
    def spotify():
        ...
        data = json.loads(MusicModule.sportifyTrendyTrack(url))
        base_row_dict={}
        data_list=[data]
        ...
        while (str(page) != 'None'):
            data = json.loads(MusicModule.sportifyTrendyTrack(page))
    ...
    def is_valid_link(url,retry_url=1):
        try:
            ...
            if(response.status_code<400):
                ...
                if(retry_url==1):
                    link_newval=url.lower().split('-sped-')
                    if(len(link_newval)>1):
                        new_url=link_newval[0]+'-lyrics'
                        return MusicModule.is_valid_link(new_url,2)
                    else:
                        return MusicModule.is_valid_link(url,2)
                elif(retry_url==2):
                    link_newval=url.lower().split('-from-')
                    if(len(link_newval)>1):
                        new_url=link_newval[0]+'-lyrics'
                    ...
                    link_newval=url.lower().split('-with-')
                    if(len(link_newval)>1):
                        new_url=link_newval[0]+'-lyrics'
                    ...
                    link_newval=url.lower().split('-single-')
                    if(len(link_newval)>1):
                        new_url=link_newval[0]+'-lyrics'
                    ...
                    link_newval=url.lower().split('-club-')
                    if(len(link_newval)>1):
                        new_url=link_newval[0]+'-lyrics'
                    ...
                    link_newval=url.lower().split('-2011-')
                    if(len(link_newval)>1):
                        new_url=link_newval[0]+'-lyrics'
...
def lyric_url(val):
    ...
    if (type(val) is str):
#         print(val," :::: ")
        return unidecode(((val.split('(')[0]+val.split(')')[1] if ( len(val.split(')'))>1 and len(val.split(')')[1])>0 ) else val.replace('(','').replace(')',''))).split('[')[0].split("feat.")[0].strip().replace(",","-and-").replace("/","").replace(".","").replace("\'","").replace(" ","-").replace("’","").replace("--","-").replace('--','-').replace('?','').replace('!','') )
    else:
        return unidecode("-".join("-and-".join(val).strip().split()).replace(".","").replace(",","").replace('\'','').replace('--','-').replace('?','').replace('!','') )

input_df['artists'] = input_df['artist_names'].apply(lyric_url)
input_df['track_names'] = input_df['track_name'].apply(lyric_url)
input_df['base_url']='https://genius.com/'
input_df['end_url']='lyrics'
input_df['artists_track'] = input_df['base_url'].str.cat(input_df['artists'].str.cat(input_df['track_names'], sep='-')).str.cat(input_df['end_url'], sep='-')
